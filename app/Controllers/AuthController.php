...
class AuthController extends Controller
{
    ...
    public function twoFactor() {
        $user = user();
        if ($user->g2fa_secret != null) 
            return redirect()->to(site_url('/login'))->with('error', "You already have a 2FA secret");

        $google2fa = model(Google2FA::class);
        ...
        $qrcode_image = "https://chart.googleapis.com/chart?chs=400x400&cht=qr&chl=$g2faUrl&choe=UTF-8";
    ...
    public function register()
    {
        ...
        $this->session->setFlashData('captcha', $timage->text);
        $data = [
            'config' => $this->config,
    ...
    public function attemptRegister()
    {
        ...
        // Check captcha
        $captcha = $this->session->getFlashData('captcha');
        if ($captcha != $this->request->getPost('captcha')) {
            return redirect()->back()->withInput()->with('error', "Captcha is not correct");
        }
